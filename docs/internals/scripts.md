...
runtime. The provided `pty.node` must have a matching `t3-resource-monitor` binary in the same
directory; packaging fails if either WSL-native asset is missing. NSIS is downloaded by
electron-builder and does not need a separate installation.
